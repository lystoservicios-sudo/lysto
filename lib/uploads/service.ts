...
export const uploadDeclaration = z
  .object({
    kind: z.enum([
      'request-photo',
    ...
    phase: z.enum(['before', 'during', 'after', 'document']).optional(),
    documentType: z
      .string()
...
const intentSchema = z.object({
  id: z.string().uuid(),
  status: z.enum(['pending', 'verified', 'cleaning', 'cleaned']),
  kind: uploadDeclaration.shape.kind,
  ownerProfileId: z.string().uuid(),
  entityId: z.string().uuid().nullable(),
  draftId: z.string().uuid().nullable(),
  expectedMimeType: uploadDeclaration.shape.mimeType,
  expectedSizeBytes: z
    .number()
    .int()
    .positive()
    .max(20 * 1024 * 1024),
  expectedSha256: uploadDeclaration.shape.sha256,
  quarantineBucket: z.literal('upload-quarantine'),
  ...
  outputPath: z
    .string()
    .regex(
      /^[a-f0-9-]{36}\/[a-f0-9-]{36}\/(?:photo\/|before\/|during\/|after\/|document\/)?[a-f0-9-]{36}\.(?:jpg|webp)$/
    ),
  outputMimeType: z.enum(['image/jpeg', 'image/webp']),
...
async function rpc(
  ...
  if (result.error) {
    ...
    if (['22023', '22P02', '23514'].includes(result.error.code ?? ''))
...
export async function signUpload(session: Session, body: unknown) {
  ...
  if (
    !validateUpload({ kind: input.kind, mimeType: input.mimeType, sizeBytes: input.sizeBytes })
      .valid
  )
    throw new ApiError('invalid_input')
  ...
  const { data, error } = await session.client.storage
    .from(intent.quarantineBucket)
    .createSignedUploadUrl(intent.quarantinePath, { upsert: false })
  ...
  return {
    ...
    path: data.path,
    token: data.token,
    finalizeBefore: intent.expiresAt
  }
}
function receipt(intent: Intent) {
  ...
  return {
    ...
    bucket: intent.outputBucket,
    path: intent.outputPath
  }
}
export async function finalizeUpload(session: Session, body: unknown) {
  ...
  const downloaded = await service.storage
    .from(intent.quarantineBucket)
    .download(intent.quarantinePath)
  ...
  try {
    inspected = await inspectUpload(new Uint8Array(await downloaded.data.arrayBuffer()), {
      mimeType: intent.expectedMimeType,
      sizeBytes: intent.expectedSizeBytes,
      sha256: intent.expectedSha256,
      outputMimeType: intent.outputMimeType
    })
  } catch (error) {
  ...
  const stored = await service.storage
    .from(intent.outputBucket)
    .upload(intent.outputPath, inspected.bytes, {
  ...
  if (stored.error) {
    ...
    if (
      existing.error ||
      !existing.data ||
      existing.data.size !== inspected.bytes.length ||
      createHash('sha256')
        .update(Buffer.from(await existing.data.arrayBuffer()))
        .digest('hex') !== inspected.sha256
    )
      throw new ApiError('service_unavailable')
  }
  const verified = await rpc(service, 'finalize_verified_upload', {
    ...
    p_actual_mime_type: intent.expectedMimeType,
    p_actual_size_bytes: intent.expectedSizeBytes,
    p_actual_sha256: intent.expectedSha256,
    p_output_size_bytes: inspected.bytes.length,
    p_output_sha256: inspected.sha256,
...
export async function readUpload(session: Session, body: unknown) {
  ...
  const { data, error } = await serviceStorage()
    .storage.from(intent.outputBucket)
    .createSignedUrl(intent.outputPath, 60, { download: filename })
