...
export function calculateProfessionalRating(
  existingAverage: number,
  existingCount: number,
  newRating: number
): { ratingAvg: number; jobsReviewed: number } {
  ...
  const ratingAvg =
    Math.round(((existingAverage * existingCount + newRating) / jobsReviewed) * 100) / 100
...
export function recalculateProfessionalRating(input: {
  ratingAvg: number | null
  reviewsCount: number
  newScore: ReviewScore
}) {
  validateReview(input.newScore)
  const average = (input.newScore.serviceRating + input.newScore.professionalRating) / 2
  return calculateProfessionalRating(input.ratingAvg ?? 0, input.reviewsCount, average)
}
