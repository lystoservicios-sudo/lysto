import { createMercadoPagoSplit, MercadoPagoOAuthHttpClient, OAuthManager, TokenCipher, type PaymentWebhookEvent, type WebhookSignatureVerificationInput } from '@waltergaltieri/mercadopago-split'
...
export async function providerJson(path: string, accessToken: string, method = 'GET', body?: unknown, idempotencyKey?: string) {
  ...
  if (!response.ok) throw new Error(response.status === 401 || response.status === 403 ? 'seller_not_linked' : 'provider_unavailable')
...
export function marketplaceGateway() {
  ...
  const split = createMercadoPagoSplit({
    ...config, storage: marketplaceStorage(), oauthHttpClient: marketplaceOAuthHttp(),
    callbacks: {
      onPaymentApproved:onPayment,onPaymentPending:onPayment,onPaymentInProcess:onPayment,onPaymentRejected:onPayment,
      onPaymentCancelled:onPayment,onPaymentRefunded:onPayment,onPaymentChargedBack:onPayment,onPaymentUnknown:onPayment,
      onMerchantOrder: async event => {
        ...
        for (const id of event.paymentIds.slice(0,50)) {
          ...
          if (typeof ref === 'string' && /^[0-9a-f-]{36}$/i.test(ref)) await applyCanonicalPayment(ref,payment,`order:${id}:${String(payment.date_last_updated)}`)
        }
      },
    },
    preferenceClientFactory: ({ accessToken }) => ({
      create: async ({ body,requestOptions }) => {
        ...
        const payload = buildPreferencePayload({ ...body },new URL(body.back_urls!.success!).origin,createdAt)
    ...
    webhookResourceClientFactory: ({ accessToken }) => ({
      ...
      getMerchantOrder: ({ merchantOrderId }) => providerJson(`/merchant_orders/${encodeURIComponent(merchantOrderId)}`,accessToken),
    }),
  })
  return split
}
export function marketplaceOAuth() {
  const config = marketplaceConfig()
  return new OAuthManager({ ...config, storage: marketplaceStorage(), tokenCipher: new TokenCipher(config.encryptionKey), httpClient: marketplaceOAuthHttp() })
}
...
export function marketplaceOAuthHttp() {
  ...
  return new MercadoPagoOAuthHttpClient({ fetch: async (input,init) => {
...
export async function createCheckoutPreference(checkout: CheckoutRow) {
  ...
  try {
    ...
    if (!account?.enabled || account.mercadoPagoUserId !== checkout.seller_account_id) throw new Error('checkout_identity_changed')
    const preference = await marketplaceGateway().payments.createPreference(claim.spec)
    const result = await paymentDatabase().query<CheckoutRow>(`update public.marketplace_checkouts set preference_id=$3,init_point=$4,sandbox_init_point=$5,
    ...
    [checkout.id,claim.token,preference.preferenceId,preference.initPoint,preference.sandboxInitPoint])
    if (!result.rows[0]) throw new Error('checkout_busy')
    return result.rows[0]
  } catch (error) {
    await paymentDatabase().query("update public.marketplace_checkouts set lease_until=null,lease_token=null,last_error='provider_request_failed' where id=$1 and lease_token=$2",[checkout.id,claim.token])
    throw error
  }
}
...
export async function reconcileCheckout(checkout: CheckoutRow, force = false) {
  const claimed = await paymentDatabase().query('update public.marketplace_checkouts set last_reconciled_at=now() where id=$1 and (last_reconciled_at is null or last_reconciled_at < now()-interval \'30 seconds\') returning id',[checkout.id])
  if (!claimed.rowCount && !force) return
  const token = await marketplaceOAuth().getValidAccessToken(checkout.professional_id)
  const search = await providerJson(`/v1/payments/search?external_reference=${encodeURIComponent(checkout.id)}&sort=date_last_updated&criteria=desc&limit=50`,token)
  ...
  if (paging?.total && paging.total > 50) {
    await paymentDatabase().query("update public.marketplace_checkouts set status='review',review_reason='too_many_payments' where id=$1",[checkout.id])
  ...
  for (const result of search.results) {
    ...
    await applyCanonicalPayment(checkout.id,raw,`reconcile:${String(raw.id)}:${String(raw.date_last_updated)}`)
  }
}
...
export async function renewCheckout(checkout: CheckoutRow) {
  ...
  return paymentTransaction(async db => {
    const result=await db.query<CheckoutRow>('select * from public.marketplace_checkouts where id=$1 for update',[checkout.id])
    const current=result.rows[0]
    if (!current?.preference_id || !['ready','expired','rejected','cancelled'].includes(current.status)) throw new Error('checkout_review')
    ...
    const unresolved=await db.query("select 1 from public.marketplace_payment_observations where checkout_id=$1 and provider_status not in ('rejected','cancelled') limit 1",[current.id])
    if (unresolved.rowCount) throw new Error('checkout_review')
    const job=await db.query("select 1 from public.jobs where id=$1 and professional_id=$2 and status::text not like 'cancelled%'",[current.job_id,current.professional_id])
    if(!job.rowCount) throw new Error('checkout_review')
    const token=await marketplaceOAuth().getValidAccessToken(current.professional_id)
    const preference=await providerJson(`/checkout/preferences/${encodeURIComponent(current.preference_id)}`,token)
    if (String(preference.collector_id)!==current.seller_account_id || preference.external_reference!==current.id) throw new Error('checkout_identity_changed')
    const expiresAt=new Date(Date.now()+30*60_000)
    await providerJson(`/checkout/preferences/${encodeURIComponent(current.preference_id)}`,token,'PUT',{
      expires:true,expiration_date_from:current.created_at.toISOString(),expiration_date_to:expiresAt.toISOString(),
    })
    await db.query("update public.marketplace_checkouts set status='ready',expires_at=$2,updated_at=now(),last_error=null where id=$1",[current.id,expiresAt])
    await db.query("insert into private.marketplace_applied_events(event_id,checkout_id) values($1,$2)",[`renew:${current.id}:${expiresAt.toISOString()}`,current.id])
  })
}
