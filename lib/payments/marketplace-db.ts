...
export async function closePaymentDatabaseForTests() {
  if (process.env.APP_ENV !== 'test') throw new Error('test_environment_required')
  const current = pool
  pool = undefined
  await current?.end()
}
export function paymentDatabase() {
  ...
  return (pool ??= new Pool({
    ...
    application_name: 'lysto-payments'
  }))
}
export async function paymentTransaction<T>(work: (client: PoolClient) => Promise<T>) {
  ...
  try {
    ...
    await client.query(
      "set local lock_timeout='5s'; set local idle_in_transaction_session_timeout='15s'"
    )
