...
import { readTrustedRole, safeLocalRedirectPath } from './session-routing'
...
export async function requirePageSession(role: UserRole): Promise<Session> {
  try {
    ...
    if (error instanceof ApiError && error.status === 401) {
      if (role === 'customer') redirect('/login')
      const destination = safeLocalRedirectPath((await headers()).get('x-lysto-path'), role)
