...
import { requireSession, requireAdminPermission, type AdminPermission, type Session } from '../auth/session'
...
export async function requirePricingPermission(session: PricingSession, permission: AdminPermission) {
...
export function pricingError(error: unknown) {
  ...
  if (error instanceof ZodError) return privateJson({ error: 'Revisá los datos del presupuesto.', code: 'invalid_input' }, { status: 400 })
  ...
  return privateJson({ error: messages[known] ?? 'No se pudo completar la operación. Revisá los datos o contactá a operaciones.', code: known in messages ? known : 'pricing_unavailable' }, { status: known.includes('configured') || known === 'pricing_database_not_ready' ? 503 : 400 })
}
