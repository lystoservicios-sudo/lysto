...
export function pricingError(error: unknown) {
  ...
  const messages: Record<string, string> = {
    new_requests_paused: 'Por el momento coordinamos las nuevas solicitudes desde Contacto.',
  ...
  return privateJson(
    ...
    { status: known.includes('configured') || known === 'pricing_database_not_ready' || known === 'new_requests_paused' ? 503 : 400 }
  )
}
