...
import ProfessionalDetailRoute from '@/app/(admin)/admin/profesionales/[id]/page'
import CustomerDetailRoute from '@/app/(admin)/admin/clientes/[id]/page'

vi.mock('next/navigation', () => ({ usePathname: () => '/admin/dashboard', notFound: () => { throw new Error('NOT_FOUND') } }))
afterEach(cleanup)

describe('administrative UI', () => {
  it.each([
    [RequestDetailRoute, 'req_1002', 'request'], [JobDetailRoute, 'job_7002', 'job'],
    [ProfessionalDetailRoute, 'pro_002', 'professional'], [CustomerDetailRoute, 'cus_002', 'customer']
  ...
  it.each([RequestDetailRoute, JobDetailRoute, ProfessionalDetailRoute, CustomerDetailRoute])('rejects unknown detail identifiers', async Page => {
    await expect(Page({ params: Promise.resolve({ id: 'missing-record' }) })).rejects.toThrow('NOT_FOUND')
  })
  ...
  it('keeps every non-detail module discoverable', () => {
    expect(new Set(adminModules.flatMap(group => group.items.map(([, route]) => route))).size).toBe(21)
  })
  ...
  it('filters requests and recovers the empty state', () => {
    ...
    fireEvent.change(screen.getByLabelText('Buscar en solicitudes'), { target: { value: 'no existe' } })
  ...
  it('links customers using their customer IDs', () => {
    ...
    expect(within(row).getByRole('link', { name: 'Ver ficha' }).getAttribute('href')).toBe('/admin/clientes/cus_002')
  })
  ...
  it('does not allow assignment of unpaid requests', () => {
    ...
    expect((screen.getByRole('button', { name: 'Guardar selección' }) as HTMLButtonElement).disabled).toBe(true)
  })
  it('saves candidate choices as explicitly local drafts', () => {
    render(<AdminWorkspace><CandidateSelector request={requests[0]} /></AdminWorkspace>)
    fireEvent.click(screen.getAllByRole('radio')[0])
  ...
  it('updates the price preview from input and priority', () => {
    render(<AdminWorkspace><PricesPage /></AdminWorkspace>)
    fireEvent.change(screen.getByLabelText('Visita de diagnóstico · ARS'), { target: { value: '40000' } })
  ...
  it('restores a saved session draft when a form remounts', () => {
    ...
    const { rerender } = render(<AdminWorkspace><SettingsForm storageKey="test" fields={fields} /></AdminWorkspace>)
    fireEvent.change(screen.getByLabelText('Nombre de prueba'), { target: { value: 'Editado' } })
    fireEvent.click(screen.getByRole('button', { name: 'Guardar borrador' }))
    rerender(<AdminWorkspace><p>Otra pantalla</p></AdminWorkspace>)
    rerender(<AdminWorkspace><SettingsForm storageKey="test" fields={fields} /></AdminWorkspace>)
  ...
  it('toggles notification preferences without sending messages', () => {
    render(<AdminWorkspace><NotificationsPage /></AdminWorkspace>)
