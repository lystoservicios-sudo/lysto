...
describe.skipIf(!process.env.LYSTO_TEST_DATABASE_URL)('marketplace ledger against PostgreSQL (rolled back)',()=>{
 beforeEach(async()=>{const url=new URL(process.env.LYSTO_TEST_DATABASE_URL!);if(!['localhost','127.0.0.1'].includes(url.hostname))throw new Error('Tests require local PostgreSQL');state.client=new Client({connectionString:url.toString()});await state.client.connect();await state.client.query('begin');await state.client.query(readFileSync('supabase/tests/fixtures/session.sql.inc','utf8'));await state.client.query(readFileSync('supabase/tests/fixtures/marketplace.sql.inc','utf8'));const jobs=await state.client.query('select id from public.jobs where customer_id=$1',[customer]);checkout=await prepareCheckout(customer,jobs.rows[0].id,undefined,false);await state.client.query("update public.marketplace_checkouts set checkout_protocol='preferences' where id=$1",[checkout.id]);checkout.checkout_protocol='preferences'})
