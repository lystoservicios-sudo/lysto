...
it('requires the complete customer profile before calculating or saving a quote', async () => {
  mocks.session.mockRejectedValue(new Error('customer_profile_incomplete'))
  const result = await POST(new Request('http://localhost/api/pricing/quote', { method: 'POST', body: '{}' }))
  expect(mocks.session).toHaveBeenCalledWith({ requireCompleteCustomer: true })
  expect(result.status).toBe(403)
  expect(await result.json()).toMatchObject({ code: 'customer_profile_incomplete', next: '/completar-perfil' })
  expect(mocks.travel).not.toHaveBeenCalled()
})
