...
const ids = { user: '00000000-0000-4000-8000-000000000001', profile: '00000000-0000-4000-8000-000000000002', entity: '00000000-0000-4000-8000-000000000003' }
...
function arrange(role: Role = 'customer', overrides: Record<string, unknown> = {}) {
  ...
  const query = { select: vi.fn().mockReturnThis(), eq: vi.fn().mockReturnThis(), single: vi.fn().mockResolvedValue({ data: { id: ids.profile, role }, error: null }) }
  mocks.createClient.mockResolvedValue({ auth: { getUser: mocks.getUser }, rpc: mocks.rpc, from: vi.fn(() => query) })
}
...
describe('current server session', () => {
  ...
  it.each(['customer', 'professional', 'admin'] as const)('resolves the current %s role entity', async role => {
    arrange(role)
    const session = await getPricingSession()
    expect(session).toMatchObject({ role, [role === 'customer' ? 'customerId' : role === 'professional' ? 'professionalId' : 'adminProfileId']: ids.entity })
  })
  ...
  it('distinguishes an unavailable Auth server from an expired login', async () => {
    mocks.getUser.mockResolvedValue({ data: { user: null }, error: { name: 'AuthRetryableFetchError', status: 503 } })
    await expect(getPricingSession()).rejects.toMatchObject({ status: 503, code: 'session_unavailable' })
  })
  it('does not elevate user-editable metadata', async () => {
    mocks.getUser.mockResolvedValue({ data: { user: { id: ids.user, app_metadata: {}, user_metadata: { app_role: 'customer' } } }, error: null })
  ...
  it('denies a profile that changed role after its JWT was issued', async () => {
    arrange('customer', { role: 'admin', customer_id: null, admin_profile_id: ids.entity, permissions: ['owner'] })
    await expect(getPricingSession()).rejects.toMatchObject({ status: 403 })
  })
  it.each([null, {}, { role: 'owner' }, { ...context(), customer_id: null }])('fails closed for incomplete context %j', async data => {
  ...
  it('requires the requested current administrative permission', async () => {
    arrange('admin')
    await expect(requireAdminPermission('operations')).resolves.toMatchObject({ permissions: ['operations'] })
  ...
  it('accepts owner for each administrative permission', async () => {
    ...
    for (const permission of ['operations', 'finance', 'quality', 'owner'] as const) {
      await expect(requireAdminPermission(permission)).resolves.toMatchObject({ permissions: ['owner'] })
    }
  })
  ...
  it.each(['suspended', 'rejected', 'under_review', 'invited'])('denies operational access to a %s professional', async professional_status => {
  ...
  it('fails closed and preserves an unavailable distinction on database failure', async () => {
    ...
    try { await getPricingSession(); expect.fail('must reject') } catch (error) {
