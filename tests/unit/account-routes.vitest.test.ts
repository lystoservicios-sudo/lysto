...
const fixtures = vi.hoisted(() => ({ verify: vi.fn(), signOut: vi.fn(), update: vi.fn(), getUser: vi.fn(), bootstrap: vi.fn() }))
vi.mock('@/lib/supabase/server', () => ({ createServerSupabaseClient: async () => ({ auth: { verifyOtp: fixtures.verify, signOut: fixtures.signOut, updateUser: fixtures.update, getUser: fixtures.getUser } }) }))
...
function post(path: string, values: Record<string,string>, requestOrigin: string | null = origin) {
  return new Request(origin + path, {method:'POST',headers:requestOrigin ? {origin:requestOrigin} : {},body:new URLSearchParams(values)})
}
beforeEach(() => {
  vi.clearAllMocks()
  vi.stubEnv('NEXT_PUBLIC_APP_URL',origin)
  fixtures.verify.mockResolvedValue({data:{user:{id:'user',app_metadata:{app_role:'customer'}}},error:null})
...
describe('account route boundaries', () => {
  it('GET previews never consume confirmation tokens or establish sessions', async () => {
    const response = await confirmGet(new Request(`${origin}/auth/confirm?token_hash=${token}&next=https://evil.test`))
  ...
  it.each([null,'https://evil.test'])('rejects confirmation without same-origin POST: %s', async source => {
    expect((await confirmPost(post('/auth/confirm',{token_hash:token},source))).status).toBe(403)
    expect(fixtures.verify).not.toHaveBeenCalled()
  })
  it('verifies only an email token and ignores attacker-supplied type/redirect', async () => {
    const response = await confirmPost(post('/auth/confirm',{token_hash:token,type:'recovery',next:'https://evil.test'}))
  ...
  it('preserves verified Auth account when profile completion remains necessary', async () => {
    ...
    expect((await confirmPost(post('/auth/confirm',{token_hash:token}))).headers.get('location')).toBe(origin + '/completar-cuenta')
  ...
  it('rejects an invalid new password before consuming recovery token', async () => {
    expect((await resetPost(post('/auth/reset-password',{token_hash:token,password:'short',repeatPassword:'short'}))).status).toBe(400)
    expect(fixtures.verify).not.toHaveBeenCalled()
  })
  it('requires a recovery token before changing password and closes sessions afterward', async () => {
    const response = await resetPost(post('/auth/reset-password',{token_hash:token,password:'ClaveNuevaLarga123!',repeatPassword:'ClaveNuevaLarga123!'}))
  ...
  it('does not change a password if the recovery token is invalid', async () => {
    fixtures.verify.mockResolvedValue({data:{user:null},error:{code:'otp_expired'}})
    expect((await resetPost(post('/auth/reset-password',{token_hash:token,password:'ClaveNuevaLarga123!',repeatPassword:'ClaveNuevaLarga123!'}))).status).toBe(400)
