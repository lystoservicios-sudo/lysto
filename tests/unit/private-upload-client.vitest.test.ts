...
const handle = {
  ...
  path: `${draftId}/${intentId}`,
  token: 'a-private-upload-token'
}
...
describe('private upload acknowledgement and retries', () => {
  it('uses the bounded onboarding routes without sending a client-only scope to the server', async () => {
    const requests = vi
      .fn()
      .mockResolvedValueOnce(json(handle))
      .mockResolvedValueOnce(
        json({
          id: intentId,
          status: 'verified',
          attachmentId: intentId,
          draftId: null,
          entityId: draftId
        })
      )
    ...
    expect(requests.mock.calls[0][0]).toBe('/api/professional/onboarding/documents/sign')
    expect(JSON.parse(requests.mock.calls[0][1].body).scope).toBeUndefined()
    expect(requests.mock.calls[1][0]).toBe('/api/professional/onboarding/documents/finalize')
    requests.mockClear()
    await expect(
      uploadPrivateFile(file, { kind: 'job-photo', scope: 'professional-onboarding' })
    ).rejects.toThrow()
  ...
  it('sends the hash without a filename and waits for server verification', async () => {
    const requests = vi
      ...
      .mockResolvedValueOnce(json(handle))
      .mockResolvedValueOnce(
        json({ id: intentId, status: 'verified', attachmentId: intentId, draftId, entityId: null })
      )
    vi.stubGlobal('fetch', requests)
    upload.mockResolvedValue({ error: null })
    const saved = vi.fn()
  ...
  it('does not announce success when inspection fails', async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValueOnce(json(handle))
        .mockResolvedValueOnce(json({ error: 'Contenido inválido' }, 400))
    )
    upload.mockResolvedValue({ error: null })
  ...
  it('rejects PDFs before calling the network', async () => {
    ...
    vi.stubGlobal('fetch', requests)
    await expect(
      uploadPrivateFile(new File(['pdf'], 'document.pdf', { type: 'application/pdf' }), {
        kind: 'professional-document',
        entityId: intentId
      })
    ).rejects.toThrow()
  ...
  it('clears an expired handle so the next retry can create a fresh intent', async () => {
    ...
    const saved = vi.fn()
    await expect(
      uploadPrivateFile(file, { kind: 'request-photo' }, saved, handle)
    ).rejects.toThrow()
