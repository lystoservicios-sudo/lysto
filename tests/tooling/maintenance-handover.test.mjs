...
test('progress reconciles all forty tasks without pending narrative completion', async () => {
  ...
  assert.equal(progress.tasks.filter(task => task.status === 'implemented').length, 6)
  assert.equal(progress.tasks.filter(task => task.status === 'in_progress').length, 19)
...
test('handover explicitly rejects production readiness without gates and pilot', async () => {
  ...
  assert.match(handover, /11 tareas verificadas, 6 implementadas.*19 en progreso/s)
