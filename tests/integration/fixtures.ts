...
export function createFixtureAccounts(
  ...
  const fetchFor =
    (setup: boolean): typeof fetch =>
    (input, init) => {
      const url = new URL(input instanceof Request ? input.url : String(input))
      if (url.origin !== target.apiUrl)
        throw new Error('Fixture HTTP requests must stay inside the disposable Supabase API')
      const originalSignal = init?.signal ?? (input instanceof Request ? input.signal : undefined)
      const signals = [originalSignal, ...(setup ? [setupController.signal] : [])].filter(
        (signal): signal is AbortSignal => Boolean(signal)
      )
      return boundedFixtureOperation(
  ...
  const cleanupAdmin = createClient(target.apiUrl, env.LYSTO_TEST_SERVICE_ROLE_KEY!, {
    auth: authOptions,
    global: { fetch: fetchFor(false) }
  })
  ...
  const record = (status: string, failures: string[] = []) =>
    writeFileSync(
      manifestPath,
      JSON.stringify(
        {
          ...
          status,
          authIds: created,
          uncertainAuthIds: [...uncertainCreations],
          failures,
  ...
  const cleanup = () => {
    ...
    cleanupPending = (async () => {
      ...
      const steps: Array<{ label: string; run: () => Promise<unknown> }> = [
        { label: 'close database', run: () => database.end() }
      ]
      steps.push({
        label: 'remove scoped operational records',
        run: async () => {
          const cleanupDatabase = new Client({
            connectionString: target.databaseUrl,
      ...
      if (uncertainCreations.size)
        failures.push('uncertain Auth creation; reconcile exact manifest IDs')
  ...
  const step = <T>(label: string, operation: () => Promise<T>) =>
    boundedFixtureOperation(label, operation, { signal: setupController.signal })
  const query = (sql: string, values?: unknown[]) =>
    step('fixture database query', () => database.query(sql, values))
  const pending = (async () => {
    try {
      await step('fixture database connection', () => database.connect())
      for (const name of accountNames) {
        const role = name.startsWith('customer')
          ? 'customer'
        ...
        try {
          await query(
            'insert into public.profiles (id,auth_user_id,role,first_name,last_name,email,phone) values ($1,$2,$3,$4,$5,$6,$7)',
            [profileId, createdAuthId, role, name, 'Synthetic', email, role === 'customer' ? '+541100000000' : null]
          )
          if (role === 'customer') {
            ...
            await query(
              'insert into public.customer_addresses (customer_id,street,number,city,province,property_type,is_default,archived_at) values ($1,$2,$3,$4,$5,$6,true,null)',
              [entityId, `Synthetic ${name}`, '1', 'CABA', 'CABA', 'apartment']
            )
          } else if (role === 'professional') {
            await query(
              'insert into public.professional_profiles (id,profile_id,status) values ($1,$2,$3)',
              [entityId, profileId, name === 'professionalApproved' ? 'approved' : 'suspended']
            )
          } else {
            await query(
              'insert into public.admin_profiles (id,profile_id,can_manage_payments,can_manage_professionals) values ($1,$2,$3,$4)',
              [
                entityId,
                profileId,
                name === 'finance' || name === 'owner',
                name === 'operations' || name === 'owner'
              ]
            )
            await query(
      ...
      try {
        ...
        throw new Error(
          'Fixture setup failed and cleanup requires reconciliation; inspect the non-secret run manifest'
        )
      }
      throw error
    }
