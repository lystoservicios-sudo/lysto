...
export const privateHttpMethods = [
  ...
  ['GET', '/api/jobs/extras'],
  ['POST', '/api/jobs/extras'],
  ['PATCH', '/api/jobs/extras'],
  ['GET', '/api/pricing/job'],
  ['POST', '/api/pricing/job/status'],
  ...
  ['PATCH', '/api/pricing/quotes'],
  ['GET', '/api/mercadopago/account'],
  ['DELETE', '/api/mercadopago/account'],
  ['GET', '/api/mercadopago/checkouts'],
  ['POST', '/api/mercadopago/checkouts'],
  ...
  ['POST', '/api/professional/onboarding/accept'],
  ['POST', '/api/professional/onboarding/submit'],
  ['POST', '/api/professional/onboarding/documents/sign'],
  ['POST', '/api/professional/onboarding/documents/finalize'],
  ['POST', '/api/professional/onboarding/documents/read'],
  ['GET', '/api/admin/professionals'],
  ['GET', '/api/admin/professionals/invitations'],
  ['GET', '/api/admin/professionals/review'],
...
describe('HTTP access control with real Auth sessions', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      for (const [index, owner] of [
        fixtures.accounts.customerA,
        fixtures.accounts.customerB
      ].entries()) {
        const inserted = await database.query(
          ...
          where c.slug='aire_acondicionado' and i.slug='mantenimiento' returning id`,
          [requestIds[index], owner.entityId]
        )
        if (inserted.rowCount !== 1)
          throw new Error('The disposable service catalog must be seeded')
        await database.query(
          ...
          [
            ...
            requestIds[index],
            owner.entityId,
            index === 0 ? fixtures.accounts.professionalApproved.entityId : null,
            index === 0 ? 'confirmed' : 'pending_assignment'
          ]
        )
      }
  ...
  async function request(
    method: string,
    path: string,
    account?: keyof NonNullable<typeof fixtures>['accounts'],
    body: unknown = {}
  ) {
  ...
  it.each(privateHttpMethods)(
    'rejects anonymous %s %s before any mutation',
    async (method, path) => {
  ...
  it.each([
    ['customerA', '/admin/dashboard'],
    ['customerA', '/pro/dashboard'],
    ['owner', '/app'],
    ['professionalSuspended', '/pro/dashboard']
  ] as const)(
    'does not grant %s the visual role of %s',
    async (account, path) => {
  ...
  it('lets an approved professional read only their assigned job', async () => {
    const assigned = await request(
      'GET',
      `/api/pricing/job?jobId=${jobIds[0]}`,
      'professionalApproved'
    )
    expect(assigned.status).toBe(200)
    const unassigned = await request(
      'GET',
      `/api/pricing/job?jobId=${jobIds[1]}`,
      'professionalApproved'
    )
  ...
  it('observes permission removal with an existing authenticated cookie', async () => {
    ...
    await database!.query(
      'delete from private.admin_profile_permissions where admin_profile_id=$1',
      [fixtures!.accounts.operations.entityId]
    )
    try {
      ...
      await database!.query(
        'insert into private.admin_profile_permissions (admin_profile_id,permission) values ($1,$2)',
        [fixtures!.accounts.operations.entityId, 'operations']
      )
    }
  }, 600_000)
  it('does not authenticate forged SSR cookie contents', async () => {
    ...
    const cookie = validCookie
      .split('; ')
      .map((entry) => `${entry.split('=')[0]}=invalid-session`)
      .join('; ')
    const response = await fetch(new URL('/api/pricing/quotes', app!.baseURL), {
      headers: { Cookie: cookie },
      signal: AbortSignal.timeout(180_000)
  ...
  it('refreshes two concurrent tabs from the same real session cookie and preserves access in both', async () => {
    ...
    ) as { expires_at: number; access_token: string; refresh_token: string }
  ...
  it('rejects forged actor fields on the implemented professional approval endpoint', async () => {
    const response = await request('POST', '/api/admin/professionals/approve', 'operations', {
      adminProfileId: 'forged',
      ownerId: 'forged'
    })
    expect(response.status).toBe(400)
  ...
  it('rejects forged ownership fields on the implemented equipment registration endpoint', async () => {
    const response = await request('POST', '/api/equipment/register', 'customerA', {
      adminProfileId: 'forged',
      ownerId: 'forged'
    })
    expect(response.status).toBe(400)
  ...
  it.each(['arbitrary-token', '00000000-0000-4000-8000-000000000000'])(
    'does not publish a sample receipt for %s',
    async (token) => {
