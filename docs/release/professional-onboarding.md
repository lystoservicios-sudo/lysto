...
5. **Aprobado** significa decisión documental humana. **Listo para trabajos nuevos** requiere además documentación vigente, foto canónica y vendedor Mercado Pago habilitado. La selección de candidatos y toda asignación nueva exigirán esos requisitos cuando Operaciones active `private.professional_readiness_rollout` después de inventariar y regularizar a los profesionales existentes; la migración deja esa restricción desactivada por defecto para no cortar asignaciones legadas. Desvincular Mercado Pago no borra trabajos ni expediente ya existentes; cualquier cobro posterior sigue sujeto a la protección financiera del checkout.
...
- Activación de la restricción: después del inventario, con la base de producción identificada y una ventana coordinada, Operaciones cambia sólo la fila `singleton` de `private.professional_readiness_rollout` a `enabled=true`. Antes de ese momento, la aptitud existente por documentación sigue aplicando a las asignaciones; no presentar el nuevo indicador como garantía de foto o Mercado Pago. La reversión de emergencia es `enabled=false` en la misma fila, conservando expedientes y trabajos.
