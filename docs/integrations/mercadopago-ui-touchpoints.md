...
| `/app/trabajos/[id]` | `DiagnosisComparison` + `CustomerApprovalPanel`; panel de pago reservado para Tanda 6 | Aprobar un importe adicional y consultar comprobante | `jobId`, `paymentId`, concepto, importe, estado, referencia de comprobante autorizada | La aprobación queda deshabilitada hasta recibir una acción explícita; no inicia un cobro ni cambia el estado localmente. |
...
- El detalle de trabajo muestra cambios de presupuesto y una aprobación deshabilitada; no crea pagos adicionales ni comprobantes.
