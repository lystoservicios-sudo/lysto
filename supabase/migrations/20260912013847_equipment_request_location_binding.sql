-- Derive service-linked equipment location from the owned, locked request.
create or replace function private.write_customer_asset(p_kind text,p_id uuid,p_expected_version integer,p_data jsonb,p_archive boolean default false) returns jsonb
...
declare
  ...
  v_id uuid; v_replaced uuid; v_access jsonb; v_category uuid; v_request uuid; v_request_row public.service_requests%rowtype; v_job public.jobs%rowtype;
begin
  ...
  if p_kind='address' then
    ...
    if exists(select 1 from jsonb_each(v_access) where jsonb_typeof(value)<>'boolean')
      ...
      or coalesce(p_data->>'propertyType','') not in ('apartment','house','commercial','office')
  ...
  if v_request is not null then
    select * into v_request_row from public.service_requests where id=v_request and customer_id=v_customer for update;
    if not found then raise exception using errcode='P0002',message='Asset not found'; end if;
    if v_request_row.equipment_id is not null then raise exception using errcode='40001',message='Request already has equipment'; end if;
    if v_request_row.address_id is null or (v_address.id is not null and v_address.id<>v_request_row.address_id) then
      raise exception using errcode='22023',message='Equipment must use the service location';
    end if;
    -- The historical address can be archived, but must remain the request's own location.
    select * into v_address from public.customer_addresses where id=v_request_row.address_id and customer_id=v_customer for share;
    if not found then raise exception using errcode='P0002',message='Service location not found'; end if;
  end if;
  select id into v_category from public.service_categories where slug='aire_acondicionado' and active;
  if v_category is null or (v_request is not null and v_request_row.category_id<>v_category) then raise exception using errcode='22023',message='Equipment category unavailable'; end if;
