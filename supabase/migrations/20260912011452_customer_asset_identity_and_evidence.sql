-- Preserve the existing property_type enum, including office, at the new boundary.
...
-- Auth alone owns the email identity. Pending email_change never updates the profile.
create function private.sync_confirmed_auth_email() returns trigger
language plpgsql security definer set search_path='' as $$
begin
...
$$;
revoke all on function private.sync_confirmed_auth_email() from public,anon,authenticated,service_role;
create trigger lysto_sync_confirmed_auth_email after update of email on auth.users
for each row execute function private.sync_confirmed_auth_email();
