-- Qualify photo JSON values and validate financial policy types at the RPC boundary.
...
begin
  ...
  select coalesce(array_agg(photo.value::uuid),'{}'::uuid[]) into selected_photos from jsonb_array_elements_text(coalesce(p_payload->'uploadIntentIds','[]'::jsonb)) as photo(value);
...
create or replace function private.update_quote_policy_v2(p_policy jsonb,p_expected_revision integer,p_reason text) returns jsonb
language plpgsql security definer set search_path='' as $$
...
begin
  ...
  if jsonb_typeof(p_policy->'version') is distinct from 'string' or jsonb_typeof(p_policy->'source') is distinct from 'string' or jsonb_typeof(p_policy->'sourceDate') is distinct from 'string' or jsonb_typeof(p_policy->'approvedUntil') not in ('string','null') then raise exception using errcode='22023',message='Policy source and dates must have the expected types'; end if;
  ...
  if exists(select 1 from jsonb_each(p_policy->'laborOverrides') item where jsonb_typeof(item.value)<>'number') then raise exception using errcode='22023',message='Numeric labor overrides required'; end if;
  ...
  insert into public.admin_audit_logs(actor_profile_id,action,entity_type,entity_id,metadata)
    values(actor,case when p_policy->>'approvedUntil' is null then 'pricing.policy.withdrawn' else 'pricing.policy.approved' end,'quote_policy',new_id,jsonb_build_object('revision',current_revision+1,'reason',trim(p_reason),'source',p_policy->>'source','approvedUntil',p_policy->>'approvedUntil'));
