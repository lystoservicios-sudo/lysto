...
select row_security_is('public','assignment_offers',true,'assignment offer RLS enabled');
select has_policy('public','assignment_offers','assignment_offer_participant_read','participants have a read policy');
...
select like((select pg_get_functiondef(p.oid) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='create_assignment_offer'),'%private.assignment_candidate_is_eligible%','transaction rechecks eligibility');
select like((select pg_get_functiondef(p.oid) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='create_assignment_offer'),'%private.insert_schedule_reservation%','offer reserves verified capacity');
select like((select pg_get_functiondef(p.oid) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='respond_assignment_offer'),'%private.professional_respond_to_job%','response reuses canonical job transition');
