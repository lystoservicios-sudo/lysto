...
insert into auth.users(id,email,raw_app_meta_data) select ('91000000-0000-0000-0000-00000000000'||n)::uuid,'pricing-'||n||'@test.local',jsonb_build_object('app_role',case when n=3 then 'professional' when n=4 then 'admin' else 'customer' end) from generate_series(1,4) n;
