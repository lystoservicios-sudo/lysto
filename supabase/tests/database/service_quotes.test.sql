...
-- This legacy fixture provides deterministic profile IDs. Signup provisioning is
-- verified independently in customer_signup.test.sql; disable only that trigger
-- while seeding these explicit profiles, then restore it within this transaction.
...
insert into public.profiles(id,auth_user_id,role,first_name,last_name,email,phone) select id,id,(raw_app_meta_data->>'app_role')::public.user_role,'Pricing','Test',email,'+54 9 11 2233 4455' from auth.users where email like 'pricing-%@test.local';
...
select id,'Corrientes','1240','CABA','Buenos Aires','apartment' from public.customer_profiles where id in ('91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000002');
...
select set_config('request.jwt.claims','{"sub":"91000000-0000-0000-0000-000000000002","app_metadata":{"app_role":"customer"}}',true);
...
select set_config('request.jwt.claims','{"sub":"91000000-0000-0000-0000-000000000001","app_metadata":{"app_role":"customer"}}',true);
...
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"91000000-0000-0000-0000-000000000003","app_metadata":{"app_role":"professional"}}',true);
