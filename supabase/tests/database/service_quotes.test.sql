...
insert into public.profiles(id,auth_user_id,role,first_name,last_name,email,phone) select id,id,(raw_app_meta_data->>'app_role')::public.user_role,'Pricing','Test',email,case when raw_app_meta_data->>'app_role'='customer' then '+541100000000' else null end from auth.users where email like 'pricing-%@test.local';
...
-- Positive quote fixtures start with the same completed profile as an onboarded customer.
insert into public.customer_addresses(customer_id,street,number,city,province,property_type,is_default,archived_at)
select id,'Corrientes','1240','CABA','Buenos Aires','apartment',true,null from public.customer_profiles where id in ('91000000-0000-0000-0000-000000000001','91000000-0000-0000-0000-000000000002');
