...
export function RegistrationForm({ policy, completing = false, next = '/app', initialValues = {} }: { policy: RegistrationPolicy; completing?: boolean; next?: string; initialValues?: { firstName?: string; lastName?: string; phone?: string } }) {
  ...
  return <form action={action} className="auth-form">
    ...
    {!completing && <><AuthInput label="Contraseña" name="password" type="password" autoComplete="new-password" minLength={12} maxLength={128} disabled={pending} /><p className="auth-help">Usá entre 12 y 128 caracteres.</p><AuthInput label="Repetir contraseña" name="repeatPassword" type="password" autoComplete="new-password" minLength={12} maxLength={128} disabled={pending} /></>}
