...
export async function GET(request: Request) {
  try {
    ...
    if (error || !job) return NextResponse.json({ error: 'No encontramos un trabajo disponible para tu cuenta.' }, { status: 404 })
    ...
    return NextResponse.json({ job, savedQuote, extras: extras ?? [], role: s.role })
