...
export async function GET(request:Request){
  try{
    ...
    const result=await paymentDatabase().query(`select c.id,c.job_id,c.extra_id,c.amount,c.marketplace_fee,c.professional_amount,c.status,c.live_mode,c.expires_at,c.review_reason,c.created_at,
    ...
    return NextResponse.json({checkouts:result.rows,role:actor.role},{headers:{'Cache-Control':'no-store'}})
  }catch(error){return paymentError(error)}
}
export async function POST(request:Request){
  try{sameOrigin(request);const actor=await paymentActor();const {checkoutId,action}=z.object({checkoutId:z.string().uuid(),action:z.enum(['reconcile','renew']).default('reconcile')}).strict().parse(await request.json());const checkout=await visibleCheckout(checkoutId,actor);if(action==='renew'){if(actor.role!=='admin')throw new Error('forbidden');await renewCheckout(checkout)}else await reconcileCheckout(checkout);return NextResponse.json({updated:true})}
  catch(error){return paymentError(error)}
}
