...
import { reviewProfessionalDocument } from '@/lib/professional/onboarding-service'
export const POST = privateRoute(
  { roles: ['admin'], permission: 'operations' },
  async (request, session) =>
    privateJson(await reviewProfessionalDocument(session, await readPrivateJsonBody(request)))
)
