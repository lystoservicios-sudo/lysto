...
const DraftContext = createContext<{
  drafts: Drafts
  save: (key: string, values: Record<string, string>) => void
}>({ drafts: {}, save: () => {} })
...
export function AdminWorkspace({ children }: { children: ReactNode }) {
  ...
  return (
    <DraftContext.Provider
      value={{
        drafts,
        save: (key, values) => setDrafts((previous) => ({ ...previous, [key]: values }))
      }}
    >
      <div className="admin-workspace">
        <div className="adm-workspace-bar">
          ...
          >
            <LayoutGrid size={16} /> Todas las herramientas <ChevronDown size={15} />
          </button>
        </div>
        {open && (
          <nav
            id="admin-tools"
            aria-label="Todas las herramientas de administración"
            className="adm-panel adm-modules"
            onKeyDown={(event) => {
              if (event.key === 'Escape') {
                setOpen(false)
                document.querySelector<HTMLButtonElement>('[aria-controls="admin-tools"]')?.focus()
              }
            }}
          >
            <div className="adm-panel-heading">
              <h2>¿Qué necesitás gestionar?</h2>
              <button
...
export function Header({
  ...
  return (
    <header className="adm-page-header">
      <div>
        {back ? (
          <Link className="adm-back" href={back.href}>
            <ArrowLeft size={15} />
            {back.label}
          </Link>
        ) : (
          <span className="adm-eyebrow">{section}</span>
        )}
        <h1>{title}</h1>
        <p>{description}</p>
      </div>
      {action && <div className="adm-header-action">{action}</div>}
    </header>
  )
}
export function Button({
  ...
  return (
    <button type="button" className={`adm-button adm-button-${variant} ${className}`} {...props}>
      {children}
    </button>
  )
}
export function ActionLink({
  ...
  return (
    <Link className={`adm-button adm-button-${primary ? 'primary' : 'secondary'}`} href={href}>
      {children}
    </Link>
  )
}
...
export function Metrics({
  ...
  items: { label: string; value: ReactNode; detail: string; icon?: ReactNode; tone?: string }[]
}) {
  return (
    <div className="adm-metrics">
      {items.map((item) => (
        <div className="adm-metric" key={item.label}>
          {item.icon && <span className={`adm-metric-icon ${item.tone ?? ''}`}>{item.icon}</span>}
          <div>
            <p>{item.label}</p>
...
export function Badge({ value }: { value: string }) {
  const tone = /cancel|suspend|critical|attention|Rechazada/i.test(value)
    ? 'red'
    : /pending|review|priority|warning|revisión|Abierta|Pendiente/i.test(value)
      ? 'amber'
      : /approved|captured|completed$|connected$|Activa|Aprobada|Resuelta/i.test(value) &&
          value !== 'not_connected'
        ? 'green'
        : 'blue'
...
export function Notice({ children, success = false }: { children: ReactNode; success?: boolean }) {
  return (
    <div className={`adm-notice ${success ? 'success' : ''}`} role={success ? 'status' : undefined}>
...
export function Tabs({
  ...
  return (
    <div className="adm-tabs" role="group" aria-label={label}>
      {options.map((option) => (
      ...
      {items.map((item) => (
        <div key={item.label}>
          <div>
            ...
            <strong>{item.detail ?? item.value}</strong>
          </div>
          <meter aria-label={item.label} min={0} max={max} value={item.value}>
            {item.value}
          </meter>
        </div>
      ))}
...
export function DataTable<T extends { id: string }>({
  ...
  search: (row: T) => string[]
  filters?: { label: string; value: (row: T) => string }[]
  title: string
  action?: ReactNode
}) {
  const [query, setQuery] = useState('')
  ...
  const filtered = rows.filter(
    (row) =>
      matchesSearch(query, search(row)) &&
      filters.every(
        (filter) => !selected[filter.label] || filter.value(row) === selected[filter.label]
      )
  )
  return (
    <section className="adm-panel adm-table-panel" aria-label={title}>
        ...
        <div className="adm-filters">
          ...
          {filters.map((filter) => (
            <select
              ...
              onChange={(event) => setSelected({ ...selected, [filter.label]: event.target.value })}
            >
              <option value="">{filter.label}: todos</option>
              {[...new Set(rows.map(filter.value))].map((value) => (
      ...
      <div aria-live="polite" className="adm-result-count" id={id}>
        {filtered.length} {filtered.length === 1 ? 'registro' : 'registros'}
      </div>
      <table className="adm-table" aria-describedby={id}>
      ...
      {filtered.length === 0 && (
        <div className="adm-empty">
          ...
          <p>Probá otro nombre o quitá los filtros.</p>
          <Button
            onClick={() => {
              setQuery('')
              setSelected({})
