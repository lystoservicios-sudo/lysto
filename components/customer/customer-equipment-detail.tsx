...
const warrantyToneStyles: Record<
  CustomerUiTone,
  { panel: string; icon: string; eyebrow: string; body: string }
> = {
  brand: {
...
export function CustomerEquipmentDetail({
  equipment,
  warranty,
  maintenance
}: {
      ...
      <Card className="overflow-hidden p-0 shadow-none">
        <div className="grid gap-5 bg-blue-50/60 p-5 sm:p-6 md:grid-cols-[15rem_minmax(0,1fr)] md:items-center">
          <EquipmentThumbnail
            ...
            equipmentName={equipment.nickname}
            size="lg"
          />
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.12em] text-blue-700">
                  Equipo {equipment.id.slice(0, 8)}
                </p>
                <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">
                  {equipment.nickname}
                </h1>
              </div>
              {equipment.statusView ? (
                <Badge tone={badgeTones[equipment.statusView.tone]}>
                  {equipment.statusView.label}
                </Badge>
              ) : null}
            </div>
        ...
        <div className="min-w-0 space-y-5">
          <Card className="shadow-none">
            ...
            <dl className="mt-4 grid gap-px overflow-hidden rounded-2xl bg-slate-100 sm:grid-cols-2">
              {technicalFacts.map((fact) => (
                <div key={fact.label} className="bg-slate-50 p-3">
            ...
            {equipment.installedAt ? (
              <p className="mt-4 flex items-center gap-2 text-sm text-slate-600">
                ...
                Instalado el {dateFormatter.format(new Date(equipment.installedAt))}
              </p>
            ) : null}
          </Card>
          ...
          <Card className="shadow-none">
            <div className="mb-4">
              ...
              <h2 className="mt-1 text-xl font-black text-slate-950">Historial de servicios</h2>
            </div>
        ...
        <aside className="min-w-0 space-y-4" aria-label="Garantía y mantenimiento del equipo">
          {warranty ? (
            <div className={cn('rounded-3xl border p-5', warrantyStyle?.panel)}>
              ...
              <p
                className={cn(
                  'mt-3 text-xs font-bold uppercase tracking-[0.12em]',
                  warrantyStyle?.eyebrow
                )}
              >
                Garantía
              </p>
              <h2 className="mt-1 text-lg font-black">{warranty.statusView.label}</h2>
              <p className={cn('mt-2 text-sm leading-6', warrantyStyle?.body)}>
                {warranty.safeSummary}
              </p>
              {warranty.coverageEndsAt ? (
                <p className="mt-3 text-xs font-semibold">
                  {warranty.status === 'expired' ? 'Venció' : 'Hasta'}{' '}
                  {dateFormatter.format(new Date(warranty.coverageEndsAt))}
                </p>
              ) : null}
            </div>
          ) : (
            <InfoNotice
              tone="info"
              title="Sin garantía vinculada"
              description="No hay una cobertura confirmada asociada a este equipo."
            />
          )}
          {maintenance ? (
            <MaintenanceReminderCard reminder={maintenance} />
          ) : (
            <InfoNotice
              tone="info"
              title="Sin próxima recomendación"
              description="El último parte no definió un mantenimiento futuro para este equipo."
            />
          )}
        </aside>
      </div>
