...
export function ProfessionalRecognitionCard({
  recognition
}: {
  ...
  return (
    <article className="rounded-3xl border border-slate-200 bg-white p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <Badge tone={recognition.verified ? 'green' : 'slate'}>
          {recognition.verified ? 'Profesional verificado' : 'Reconocimiento registrado'}
        </Badge>
      </div>
      ...
      <div className="mt-4 grid grid-cols-3 gap-px overflow-hidden rounded-2xl bg-slate-100">
        <div className="bg-slate-50 p-3">
          ...
          <p className="mt-1 text-lg font-black tabular-nums text-slate-950">
            {recognition.rating.toFixed(1)}
          </p>
        </div>
        <div className="bg-slate-50 p-3">
          <p className="text-xs font-semibold text-slate-500">Aceptación</p>
          <p className="mt-1 text-lg font-black tabular-nums text-slate-950">
            {recognition.acceptanceRate}%
          </p>
        </div>
        <div className="bg-slate-50 p-3">
          <p className="flex items-center gap-2 text-xs font-semibold text-slate-500">
