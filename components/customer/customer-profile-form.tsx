...
function validateProfile(value: CustomerProfileViewModel): ProfileErrors {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value.email.trim())) errors.email = 'Ingresá un email válido.'
  if (value.phone.replace(/\D/g, '').length < 8) errors.phone = 'Ingresá un teléfono válido.'
  return errors
}

export function CustomerProfileForm({ initialValue, onSubmit }: {
  ...
  const changeField = <K extends keyof CustomerProfileViewModel>(field: K, nextValue: CustomerProfileViewModel[K]) => {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmission({ state: 'error', message: 'No pudimos guardar los cambios. Intentá nuevamente.' })
    }
  }

  const field = (name: keyof ProfileErrors, label: string, type: 'text' | 'email' | 'tel' = 'text') => {
  ...
  return (
    <form onSubmit={submit} className="space-y-6 rounded-[2rem] border border-slate-200 bg-white p-5 sm:p-6" noValidate>
      <div>
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600">Esta información se usará para coordinar visitas y avisos importantes.</p>
      </div>
      ...
      <fieldset>
        <legend className="text-sm font-semibold text-slate-800">Preferencia de notificaciones</legend>
        <div className="mt-2 grid gap-3 sm:grid-cols-3">
          {([
            ...
            <label key={option} className="flex min-h-12 cursor-pointer items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 text-sm font-semibold text-slate-800 has-[:checked]:border-blue-300 has-[:checked]:bg-blue-50">
              <input type="radio" name="notificationPreference" value={option} checked={value.notificationPreference === option} onChange={() => changeField('notificationPreference', option)} className="h-4 w-4 accent-blue-600" />
              {label}
            </label>
          ))}
        </div>
      ...
      {!onSubmit ? <InfoNotice title="Guardado pendiente" description="El guardado del perfil se conectará cuando esté disponible la persistencia." /> : null}
