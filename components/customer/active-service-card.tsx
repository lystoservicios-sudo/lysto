  ...
  return (
    <div className="flex min-w-0 items-center gap-3 rounded-2xl bg-slate-50 p-3">
      ...
      <span className="min-w-0">
        ...
        <span className="mt-0.5 block truncate font-black text-slate-950">
          {name ?? 'Asignación pendiente'}
        </span>
        {specialty ? (
          <span className="mt-0.5 block text-xs text-slate-600">{specialty}</span>
        ) : null}
        {rating !== undefined || license ? (
          <span className="mt-1 block text-xs text-slate-500">
            {rating !== undefined ? `${rating.toLocaleString('es-AR')} de 5` : null}
            {rating !== undefined && license ? ' · ' : null}
            {license}
          </span>
        ) : null}
        {name ? (
          <span className="mt-1 block text-xs font-semibold text-slate-500">
            {verified ? 'Identidad verificada' : 'Profesional asignado'}
          </span>
        ) : null}
      </span>
    </div>
...
export function TechnicianEtaCard({
  scheduledAt,
  timeWindow
}: {
  scheduledAt: string
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-blue-100 bg-blue-50 p-3" aria-live="polite">
      ...
      <time
        ...
        className="mt-1 block text-xl font-black tabular-nums text-blue-950"
      >
        {timeWindow}
      </time>
      <p className="mt-1 text-xs leading-5 text-blue-800">
        El rango se actualizará cuando exista información nueva.
      </p>
    </div>
  )
}
    ...
    <article className="rounded-[2rem] border border-slate-200 bg-white p-5 sm:p-6">
      ...
      <div className="mt-5 flex flex-col gap-3 border-t border-slate-100 pt-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-slate-600">
          <strong className="text-slate-950">Próximo paso:</strong> {job.nextStep}
        </p>
        <ButtonLink href={`/app/trabajos/${job.id}`} variant="secondary">
          Seguir servicio
        </ButtonLink>
      </div>
    </article>
