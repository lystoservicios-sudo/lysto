      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">{request.id} · Demostración</p>
            <h2 className="mt-2 text-xl font-black tracking-tight text-slate-950">{request.issueLabel}</h2>
          </div>
          <Badge tone={badgeTones[request.statusView.tone]}>{request.statusView.label}</Badge>
        </div>

        <div className="mt-4 grid gap-2 text-sm text-slate-600 sm:grid-cols-2">
          <p className="flex min-w-0 items-start gap-2"><MapPin aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0" /><span>{request.address}</span></p>
          <p className="flex min-w-0 items-start gap-2"><CalendarClock aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0" /><span>{request.preferredWindow}</span></p>
          <p className="flex min-w-0 items-start gap-2"><Camera aria-hidden="true" className="mt-0.5 h-4 w-4 shrink-0" /><span>{request.mediaCount} {request.mediaCount === 1 ? 'archivo seleccionado' : 'archivos seleccionados'}</span></p>
        </div>
        ...
        <div className="mt-5 flex items-end justify-between gap-4 border-t border-slate-100 pt-4">
          ...
          <div className="flex shrink-0 items-center gap-3">
            {request.preliminaryPrice !== null ? <span className="hidden font-black tabular-nums text-slate-950 sm:block">{moneyFormatter.format(request.preliminaryPrice)}</span> : null}
