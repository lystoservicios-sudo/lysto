    ...
    <div className="space-y-5">
      <Card className="overflow-hidden p-0 shadow-none">
        <div className="border-b border-slate-100 bg-blue-50/60 p-5 sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.12em] text-blue-700">{job.id} · Demostración</p>
              <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">{job.issueLabel}</h1>
            </div>
        ...
        <dl className="grid gap-px bg-slate-100 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <Fact icon={CalendarClock} label="Visita" value={`${dateFormatter.format(new Date(job.scheduledAt))} · ${job.timeWindow}`} />
          <Fact icon={MonitorCog} label="Equipo" value={job.equipmentName ?? 'Por confirmar'} />
          <Fact icon={ReceiptText} label="Importe informado" value={job.finalAmount != null ? currencyFormatter.format(job.finalAmount) : job.amount != null ? currencyFormatter.format(job.amount) : 'Pendiente'} />
        </dl>
      </Card>

      <Card className="shadow-none">
        <div className="mb-4">
          <p className="text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Recorrido del servicio</p>
          <h2 className="mt-1 text-xl font-black text-slate-950">Ocho etapas, un próximo paso claro</h2>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_20rem] xl:items-start">
        ...
        <aside className="min-w-0 space-y-4" aria-label="Información del profesional y seguimiento">
          <TechnicianProfileCard name={job.professionalName} specialty={job.professionalSpecialty} rating={job.professionalRating} license={job.professionalLicense} verified={job.professionalVerified} />
...
function Fact({ icon: Icon, label, value }: { icon: typeof MapPin; label: string; value: string }) {
  return <div className="min-w-0 bg-white p-4 sm:p-5"><dt className="flex items-center gap-2 text-xs font-semibold text-slate-500"><Icon aria-hidden="true" className="h-4 w-4" />{label}</dt><dd className="mt-2 text-sm font-bold leading-6 text-slate-950">{value}</dd></div>
}
